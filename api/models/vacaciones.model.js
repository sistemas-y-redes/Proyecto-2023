...
vacacionesModel.newVacaciones = async (req) => {
  ...
  try {
    ...
    let vacaciones = await vacacionesModel.getvacacionesByCuentaFM(req.Tec);
    let ultimoRegistroVacaciones;

    if (vacaciones && vacaciones.length > 0) {
      ultimoRegistroVacaciones = vacaciones.reduce((max, vacacion) =>
        (max.recordId > vacacion.recordId) ? max : vacacion, vacaciones[0]);
    } else {
    ...
    let vacacionesFirma = {
      "fieldData": {
        "AcciónPersona": ultimoRegistroVacaciones.fieldData.CuentaFM,
        "Acción": ultimoRegistroVacaciones.fieldData.Estado,
        "Notas": ultimoRegistroVacaciones.fieldData.Notas,
        "ID": ultimoRegistroVacaciones.fieldData.IdLin
      }
    }

    let respuesta2 = await axios.post(
      `https://${serverName}/fmi/data/v1/databases/Acceso/layouts/VacacionesFirmas/records`,
      vacacionesFirma,
      {
        httpsAgent: httpsAgent,
        headers: {
