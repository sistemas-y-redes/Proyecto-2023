  ...
  <div class="nuxt-content">
    ...
    <div v-if="error">
      ...
      <div class="mostrar-error">
        ...
        <NuxtLink to="/login" class="enlace-login"
          ><button>Iniciar sesión</button></NuxtLink
        >
      </div>
    </div>
    ...
    <div v-else>
      <!-- Botón de retorno -->
      <div v-if="!FormActive" class="boton-retorno mb-4">
        <NuxtLink :to="`/${$route.params.id.replace('/', '%2F')}`"
          >&lt; <span>VOLVER</span></NuxtLink
        >
      </div>
          ...
          <div>
            ...
            <div class="opciones-pestañas">
              <b-col
                class="pestaña"
                :class="
                  this.pestañaActiva === 'Histórico' ? 'active' : 'inactive'
                "
                @click="cambiarPestaña('Histórico')"
                ><b-icon icon="clock"></b-icon> Histórico</b-col
              >
              <b-col
                class="pestaña"
                :class="
                  this.pestañaActiva === 'Adjuntos' ? 'active' : 'inactive'
                "
                @click="cambiarPestaña('Adjuntos')"
                ><b-icon icon="file-earmark"></b-icon> Adjuntos</b-col
              >
              <b-col
                class="pestaña"
                :class="
                  this.pestañaActiva === 'Materiales' ? 'active' : 'inactive'
                "
                @click="cambiarPestaña('Materiales')"
                ><b-icon icon="file-earmark"></b-icon> Materiales</b-col
              >
            </div>
            ...
            <!-- Contenido de seguimientos -->
              ...
              >
                ...
                <div
                  v-for="(element, index) in this.$store.state.Horas"
                  :key="index"
                ...
                <!-- Contenido de seguimientos -->
                <div
                  v-for="(Linea, index) in visita.VisitasLineas"
                  :key="index"
                  class="historico row"
                  v-if="
                    Linea['VisitasLineas::Tipo'] === 'M.Obra' ? true : false
                  "
                >
                ...
                <div class="boton-carga-parent">
                  <button
                    v-if="limite < visita.VisitasLineas.length"
                    style="z-index: 9"
                    class="boton-carga"
                    @click="limite += 5"
                  >
                  ...
                  <span
                    class="ml-1"
                    v-if="limite >= visita.VisitasLineas.length"
                    >Límite {{ historicoLength() }} registros alcanzado</span
                  >
            ...
            <div class="div-pestaña" v-if="this.pestañaActiva === 'Adjuntos'">
              <b-row
                v-if="visita.VisitasFotos.length > 0"
                class="lista-adjuntos"
              >
                <Gallery
                  :images="visita.VisitasFotos"
                  @deleteFile="getVisita()"
                />
                <div class="boton-carga-parent">
                  <span
                    class="ml-1"
                    v-if="limiteDocumentos >= visita.VisitasFotos.length"
                    >Límite {{ visita.VisitasFotos.length }} registros
                    alcanzado</span
                  >
                </div>
              </b-row>
            ...
            <div class="div-pestaña" v-if="this.pestañaActiva === 'Materiales'">
              <b-row
                v-if="checkMateriales() || this.$store.state.Horas.length > 0"
                class="lista-historica"
              >
                <div
                  v-for="(Linea, index) in visita.VisitasLineas"
                ...
                <div class="boton-carga-parent">
                  ...
                  <span
                    class="ml-1"
                    v-if="limite >= visita.VisitasLineas.length"
                    >Límite {{ materialesLength() }} registros alcanzado</span
                  >
      ...
      >
        <label>
          ...
          <b-form-file
            style="display: none"
            :state="Boolean(archivoSubir)"
            accept=".jpg, .png, .pdf, .webp, .jpeg"
            id="archivo"
            type="file"
            name="archivo"
            v-model="archivoSubir"
            @change="crearDocumento"
          />
        </label>
      </div>
...
export default {
  ...
  methods: {
    cambiarPestaña(PestañaSeleccionada) {
      ...
      console.log(this.pestañaActiva);
    },
